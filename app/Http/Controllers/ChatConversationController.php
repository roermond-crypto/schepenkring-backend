...
class ChatConversationController extends Controller
{
    public function index(Request $request, ChatAccessService $access)
    {
        ...
        if ($request->filled('harbor_id')) {
            $query->where('harbor_id', (int) $request->query('harbor_id'));
        }
    ...
    public function store(Request $request, ChatConversationService $service)
    {
        ...
        if (!empty($payload['session_jwt'])) {
            try {
                $decoded = json_decode(\Illuminate\Support\Facades\Crypt::decryptString($payload['session_jwt']), true);
    ...
    public function update(Request $request, string $id, ChatAccessService $access, ChatConversationService $service)
    {
        $user = $request->user();
        if (!$user) {
            return response()->json(['message' => 'Unauthorized'], 401);
        ...
        $conversation = $service->updateConversation($conversation, $payload, $user);
