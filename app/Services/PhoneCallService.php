...
class PhoneCallService
{
    ...
    public function handleCallAnswered(array $payload, ?string $occurredAt = null): void
    {
        ...
        if ($session->conversation_id) {
            $this->recordEvent($session->conversation_id, 'call_answered', [
    ...
    public function handleCallEnded(array $payload, ?string $occurredAt = null, ?string $eventType = null): void
    {
        ...
        $outcome = $session->answered_at ? 'completed' : 'missed';
        ...
        $this->recordEvent($session->conversation_id, 'call_ended', [
            'call_session_id' => $session->id,
    ...
    public function handleRecordingSaved(array $payload): void
    {
        ...
        $this->recordEvent($session->conversation_id, 'call_recording_saved', [
            'call_session_id' => $session->id,
    ...
    public function initiateOutboundCall(Message $message): void
    {
        ...
        $channel = $this->resolveHarborChannelForHarbor($conversation->harbor_id);
        ...
        if (!$this->hasMinimumBalance($conversation->harbor_id)) {
            $message->status = 'failed';
            $message->metadata = array_merge($message->metadata ?? [], ['error' => 'insufficient_balance']);
            $message->save();
            return;
        }
        ...
        $this->recordEvent($conversation->id, 'call_outbound_initiated', [
            'call_session_id' => $session->id,
            'call_control_id' => $callControlId,
        ]);
    }
    ...
    private function handleInboundCall(CallSession $session, array $payload, ?string $fromNumber, ?string $toNumber): void
    {
        ...
        if (!$this->hasMinimumBalance($channel->harbor_id)) {
            $this->rejectCall($session, 'insufficient_balance');
            return;
        }
        ...
        if ($conversation) {
            ...
            $this->recordEvent($conversation->id, 'call_initiated', [
                'call_session_id' => $session->id,
    ...
    private function rejectCall(CallSession $session, string $reason): void
    {
        ...
        if ($session->conversation_id) {
            $this->recordEvent($session->conversation_id, 'call_rejected', [
                'call_session_id' => $session->id,
                'reason' => $reason,
            ]);
        }
    ...
    private function hasMinimumBalance(?int $harborId): bool
    {
        if (!$harborId) {
            return false;
        }

        $min = (float) config('voice.min_balance_eur', 1.0);
        $balances = $this->wallet->computeBalances($harborId, 'EUR');

        return ($balances['available'] ?? 0.0) >= $min;
    }
    ...
    private function recordEvent(?string $conversationId, string $type, array $payload): void
    {
        ...
        $this->chatService->recordEvent($conversation, $type, $payload);
    }
    ...
    private function createSummaryMessage(CallSession $session): void
    {
        ...
        $this->createSystemMessage($conversation->id, $label, 'call_summary', [
            'call_session_id' => $session->id,
            'duration_seconds' => $session->duration_seconds,
            'cost_eur' => $session->cost_eur,
        ]);
