...
class CloudinaryEnhanceService
{
    ...
    {
        $this->enabled = (bool) env('CLOUDINARY_ENHANCE_ENABLED', false);
     ...
     *
     * Pipeline: upload → e_improve → e_auto_color → e_sharpen → optional e_upscale → download → delete remote
     *
     * @param string $localPath    Absolute path to the image file
     * @param array  $qualityFlags Quality flags from ImageQualityService (too_dark, blurry, low_res, etc.)
     * @param int    $rotationAngle Explicit rotation angle (0, 90, 180, 270) to apply
    ...
    public function enhance(string $localPath, array $qualityFlags = [], int $rotationAngle = 0): ?string
    {
        ...
        try {
            ...
            // All effects must be comma-separated in a single transformation
            // We use advanced AI tools: outdoor optimization, auto-color correction, and EXIF auto-rotation
            $eagerParts = ['e_improve:outdoor', 'e_auto_color'];

            if ($rotationAngle > 0 && in_array($rotationAngle, [90, 180, 270])) {
                // Apply specific rotation if AI decided it needs it
            ...
            // ── 1. Upload + process in one call (eager = synchronous server-side) ──
            $uploadResult = $this->cloudinary->uploadApi()->upload($localPath, [
                'resource_type' => 'image',
                'folder'        => 'nauticsecure_temp',
                'overwrite'     => true,
                'eager'         => [$eagerTransform],
                'eager_async'   => false,  // Wait for the transformation to complete
            ]);
