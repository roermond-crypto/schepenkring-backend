...
class EnhanceYachtImageJob implements ShouldQueue
{
    ...
    public function handle(
        ...
        if (!$enhancer->isAvailable()) {
            Log::info("[EnhanceJob] Cloudinary not available, skipping image #{$this->yachtImageId}");
            return;
        }
        ...
        try {
            ...
            if ($rotationAngle === 0 && $isGoodQuality && !$hasFlags) {
                Log::info("[EnhanceJob] Image #{$this->yachtImageId} is already good quality and perfectly straight. Skipping Cloudinary.");
                $image->update(['enhancement_method' => 'none']);
                return;
            }

            // Send to Cloudinary for AI enhancement (passing the explicit rotation angle)
