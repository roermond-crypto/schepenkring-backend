...
return [
    ...
    */
    ...
    'mailers' => [
        ...
        'smtp' => [
            'transport' => 'smtp',
            // MAIL_SCHEME (new) takes priority; fall back to MAIL_ENCRYPTION (old key) for VPS backwards-compat.
            // ssl → smtps (port 465), tls → smtp+starttls (port 587), anything else → null (plain)
            'scheme' => env('MAIL_SCHEME') ?: (
                env('MAIL_ENCRYPTION') === 'ssl'  ? 'smtps' : (
                env('MAIL_ENCRYPTION') === 'tls'  ? 'smtp+starttls' : null)
            ),
            'url' => env('MAIL_URL'),
